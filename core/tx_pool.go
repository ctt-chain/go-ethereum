...
var (
	...
	ErrPermissionSender = errors.New("invalid permission")
...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, chain blockChain ,peerId string) *TxPool {
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if tx.Type() !=0{
		//tx.getFrom
		if ! bytes.Equal(pool.currentState.GetPeerId(from), []byte(pool.peerId)){
			return ErrPermissionSender
		}
	}
