...
func (self *StateDB) GetMinerLevel(addr common.Address) uint64 {
	stateObject := self.getStateObject(addr)
	if stateObject != nil {
		return stateObject.MinerLevel()
	}
	return 0
}


func (self *StateDB) GetPeerId(addr common.Address) []byte {
	stateObject := self.getStateObject(addr)
	if stateObject != nil {
		return stateObject.PeerId()
	}
	return nil
}
...
func (self *StateDB) SetMinerLevel(addr common.Address, minerLevel uint64) {
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetMinerLevel(minerLevel)
	}
}

func (self *StateDB) SetPeerId(addr common.Address, peerId []byte) {
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetPeerId(peerId)
	}
}
